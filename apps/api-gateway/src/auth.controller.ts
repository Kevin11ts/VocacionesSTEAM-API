...
export class AuthGatewayController {
  ...
  async verifyLogin(@Body() verifyLoginDto: VerifyLoginDto) {
    ...
    return lastValueFrom(
      this.authClient.send({ cmd: 'auth.verify-otp' }, payload),
    );
  }
  ...
  @ApiResponse({
    status: 200,
    description: 'OTP sent to the registered email if it exists.',
  })
  @ApiBody({ type: ForgotPasswordDto })
  async forgotPassword(@Body() forgotPasswordDto: ForgotPasswordDto) {
    return lastValueFrom(
      this.authClient.send({ cmd: 'auth.forgot-password' }, forgotPasswordDto),
    );
  }
  ...
  async resetPassword(@Body() resetPasswordDto: ResetPasswordDto) {
    return lastValueFrom(
      this.authClient.send({ cmd: 'auth.reset-password' }, resetPasswordDto),
    );
  }
  ...
  async googleAuthRedirect(@Req() req: Request, @Res() res: Response) {
    const oauthRes = await lastValueFrom(
      this.authClient.send({ cmd: 'auth.oauth-login' }, req.user),
    );
