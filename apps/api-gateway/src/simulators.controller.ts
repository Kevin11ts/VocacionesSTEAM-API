import { Controller, Get, Post, Put, Delete, Param, Body, UseGuards, Inject, Request } from '@nestjs/common';
...
export class SimulatorsController {
  ...
  @Get()
  async getSimulators() {
    return lastValueFrom(this.testsClient.send({ cmd: 'tests.get-simulators' }, {}));
  }

  @Get(':id')
  async getSimulatorById(@Param('id') id: string) {
    return lastValueFrom(this.testsClient.send({ cmd: 'tests.get-simulator-by-id' }, { id }));
  }
  ...
  async submitSimulator(@Param('id') simulatorId: string, @Body() data: any, @Request() req: any) {
    return lastValueFrom(this.testsClient.send({ cmd: 'tests.evaluate-simulator' }, {
...
export class AdminSimulatorsController {
  ...
  async createSimulator(@Body() data: any) {
    return lastValueFrom(this.testsClient.send({ cmd: 'tests.create-simulator' }, data));
  }
  ...
  async updateSimulator(@Param('id') id: string, @Body() data: any) {
    return lastValueFrom(this.testsClient.send({ cmd: 'tests.update-simulator' }, { id, data }));
  }
  ...
  async deleteSimulator(@Param('id') id: string) {
    return lastValueFrom(this.testsClient.send({ cmd: 'tests.delete-simulator' }, { id }));
  }
}
